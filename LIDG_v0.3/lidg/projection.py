...
class Projection:
    ...
    def vector_projection(self,x_df,y_se,sort,normalize,n_round=5):
        ...
        results_df = pd.DataFrame({"Label":labs_list,"b":np.round(beta,nr),
                                   ...
                                   "-log10(p)":np.round(-log10p,6),
                                   "G2":np.round(giy2_list,6),"TRi2":np.round(tri2_list,10)},
                                   columns=["Label","b","|b|","-log10(p)","G2","TRi2"],index=range(1,n+1))
        if not sort == None:
    ...
    def outlier(self,x_df,y_se,ntop):
        ...
        print("Outliers (only show top "+str(ntop)+" sorted by |DFFITS|)")
        outlier_list = []
        for i in range(1,m+1):
            outlier_list.append([dff_sind[-i],np.round(np.abs(dff[dff_sind[-i]]),4),np.round(lev[dff_sind[-i]],4)])

        results_df = pd.DataFrame(outlier_list,
                                  columns=["Index","|DFFITS|","Leverage"],
                                  index=range(m))
