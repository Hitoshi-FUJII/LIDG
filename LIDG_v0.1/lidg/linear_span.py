...
import lidg.model_selection as ms
...
# === Model selection ===        
    def elastic_net(self,l1r=1.0,eps=0.001,kf=20,intercept=True,norm=False,itr=1000,zero=0.001):
        ...
        x_df = x_df.drop(const_list,axis=1)
        print(f"Constant descriptors {const_list} are ignored.")
        ms.elastic_net(x_df,self.y,l1r,eps,kf,intercept,norm,itr,zero)
        
        
    def ex_search(self,k,ntop=20,const=False):
        ms.exhaustive_search(self.x,self.y,k,ntop,const)
