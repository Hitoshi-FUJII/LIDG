...
# === Multicollinearity ===    
    def lid(self,spn_name="S_lid",normalize=False,tole1=None,tole2=None):         
        ...
        if normalize: print(f"  coefficients for normalized descriptors (for a fair comparison of weights)\n")
        else: print(f"  coefficients for descriptors with original scale (to obtain exact MCL relationships)\n")
        na = self._get_n()
...
# === Linear regresson and finding near multicollinearities) ===
    def project(self,sort=None,normalize=False):
        print(f"Ordinary Least Squares regression:")
        print(f"  normalize = {normalize}")
        if normalize: print(f"  coefficients for normalized descriptors (for a fair comparison of weights)\n")
        else: print(f"  coefficients for descriptors with original scale\n")
        self.pro.vector_projection(self.x,self.y,sort,normalize)
        self.hy = self.pro.hy
        form = mcl.model_form(self.hy.name,self.x.columns,self.pro.beta)
        print(f"\n{form}\n")
    ...
    def enet(self,almax=2,almin=-4,l1r=1.0,nf=10,intercept=True):
        ...
        x_df = x_df.drop(const_list,axis=1)
        print(f"Constant descriptors {const_list} are ignored.")
        coe,cri,self.hy = skl.elastic_net(x_df,self.y,almax,almin,l1r,nf,intercept)
        print(f"{cri}\n")
        print(f"{coe}")
        form = mcl.model_form(self.hy.name,list(coe["Label"]),list(coe["b"]))
        print(f"\n{form}\n")
