    ...
    def show(self):
        ...
        for i,lab in enumerate(self.x.columns):
            if lab in self.lcal.label_dic:
                print(f"{i+1: >5}  {lab}      (  =  {self.lcal.label_dic[lab]}  )")
            else:
                print(f"{i+1: >5}  {lab}")  
        print()
    ...
    def _get_inf(self):
        ...
        for i in range(len(self.x.columns)):
            for j in range(len(self.x.index)):
                if np.abs(self.x.iloc[j][i]) >= inf:
                    inf_list.append(self.x.columns[i])
        inf_list = list(sorted(set(inf_list), key=inf_list.index))
        return inf_list
...
# === Plotting ===
    def plot(self,df,se=None):
        plots.preplot(df,se)      
    
    
    def vs_plot(self,x1,x2,pmin=None,pmax=None):
        plots.vs_plot(x1,x2,pmin,pmax)      
