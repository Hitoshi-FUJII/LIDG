...
#  === Elastic net ===
def elastic_net(x_df,y_se,l1r,eps,kf,intercept,itr,zero):
    ...
    enet_cv = ElasticNetCV(l1_ratio=l1r,eps=eps,cv=kf,fit_intercept=intercept,max_iter=itr).fit(X,y)
    ...
    for alp in enet_cv.alphas_:
        enet = ElasticNet(alpha=alp,l1_ratio=l1r,fit_intercept=intercept,max_iter=itr).fit(X,y)
        r2 = enet.score(X,y)
        mse = (1.- r2)*syy/m
        mse_list.append(mse)
    ...
    enet = ElasticNet(alpha=a1se,l1_ratio=l1r,fit_intercept=intercept,max_iter=itr).fit(X,y)
