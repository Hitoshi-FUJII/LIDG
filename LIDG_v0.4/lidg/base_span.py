...
class BaseSpan:
    ...
    def read(self,file_or_df,sep="\s+",index_col=None):
        if type(file_or_df) == str:
            print(f".read: text file")
            #df = pd.read_csv(file_or_df,delim_whitespace=True,index_col=0)
            df = pd.read_table(file_or_df,sep=sep,index_col=index_col)
        else:
